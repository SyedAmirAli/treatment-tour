...
export default function HeaderMain() {
    ...
    <header className="container">
        <nav className="w-full flex justify-between items-center px-2 sm:px-0">
            ...
            <div className="flex gap-10">
                {/* Removed Toll Free Section */}
                
                <div className="flex items-center gap-4 sm:gap-6">
                    {/* Become Our Partner Button */}
                    <Link
                        href="#"
                        className="text-sm bg-primary text-white px-4 py-2 rounded-full font-bold  hover:bg-black-700 duration-300">
                        Become Our Partner
                    </Link>
    ...
    {search.status && <SearchModal />}
            ...
            <div className={`w-full ${activeNav ? "block" : "hidden"} lg:block  py-4 mt-20`}>
                <Menu />
            </div>
        </>
    );
